    ...
    $.extend({
        ...
        operate: {
            ...
            remove: function(id, title) {
            	...
            	$.modal.confirm("确定" + tst + "该条" + table.options.modalName + "信息吗？", function() {
         ...
         //复杂逻辑展示tab页面
            complexTab: function(title,id) {
            	table.set();
            	$.modal.openTab(title, $.operate.editUrl(id));
            },
            //多参数 get 新TAb
            complexUrlTab: function(title,url) {
            	table.set();
            ...
            //查看详情Tab 显示
            detailTab: function(id, title){
            	table.set();
            	let t = $.common.isEmpty(title)?"查看" + table.options.modalName:title
            	$.modal.openTab(t, $.operate.detailUrl(id));
            },
            // 保存信息 刷新表格
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.Ebank_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
